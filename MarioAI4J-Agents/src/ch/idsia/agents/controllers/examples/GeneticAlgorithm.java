...
public class GeneticAlgorithm extends MarioHijackAIBase implements IAgent {
...
//				}
		...
		if(p1.size() == p2.size()){
			for(int x =0 ; x < p1.size() ; ++x){
				...
				if(!Node1.enemyHere && Node1.xPos >= 0){
					ret.add(gP1.y.x);
...
//				System.out.println(randAct.y.x);
				++runs;
			}
//			System.out.println(possiblePath.size());
			toChoose.add(possiblePath);
		}
//		System.out.println(toChoose.size() + " chooz");
...
//		return action;
	...
	public static void main(String[] args){
		...
		IAgent agent = new GeneticAlgorithm();
		simulator.run(agent);
